...
class Docs extends CI_Controller
{
	...
	public function exportpegawai()
	{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('layout/navbar', $data);
            $this->load->view('layout/sidebar', $data);
            $this->load->view('admin/exportpegawai', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $querydata = $this->db->get_where('user')->result();
            if ($this->input->post('method_export_file') === 'pdf') {
                $data = [
                    'dataapp' => $this->get_datasetupapp,
                    'datapegawai' => $querydata
                ];
                ob_clean();
                $mpdf = new \Mpdf\Mpdf();
                $html = $this->load->view('layout/datapegawai/printallpegawai', $data, true);
                //$pdfFilePath = "storage/pdf_cache/absensipegawai_" . time() . "_download.pdf";
                $stylesheet = file_get_contents(FCPATH . 'assets/css/mpdf-bootstrap.css');
                $mpdf->WriteHTML($stylesheet, \Mpdf\HTMLParserMode::HEADER_CSS);
                $mpdf->WriteHTML(utf8_encode($html), \Mpdf\HTMLParserMode::HTML_BODY);
                $mpdf->SetTitle('Cetak Data Pegawai');
                //$mpdf->Output(FCPATH . $pdfFilePath, "F");
                $mpdf->Output("datapegawai_" . time() . "_bulanan" . "_download.pdf", "I");
            } elseif ($this->input->post('method_export_file') === 'excel') {
                ...
                $sheet->setCellValue('A1', 'Rekap Data Pegawai: ' . $dataapp['nama_instansi'] . '');
                $sheet->mergeCells('A1:H2');
                $sheet->getStyle('A1')->applyFromArray($styleJudul);
                $sheet->setCellValue('A3', 'Excel was generated on ' . date("Y-m-d H:i:s") . '');
                $sheet->mergeCells('A3:H3');
                ...
                $sheet->setCellValue('F5', 'Bagian Shift');
                $sheet->getStyle('A5:F5')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                $datapegawai = $querydata;
                $no = 1;
                $rowx = 6;
                foreach ($datapegawai as $rowpegawai) {
                    $sheet->setCellValue('A' . $rowx, $no++);
                    $sheet->setCellValue('B' . $rowx, $rowpegawai->nama_lengkap);
                    $sheet->setCellValue('C' . $rowx, $rowpegawai->kode_pegawai);
                    $sheet->setCellValue('D' . $rowx, $rowpegawai->role_id);
                    $sheet->setCellValue('E' . $rowx, $rowpegawai->jabatan);
                    $sheet->setCellValue('F' . $rowx, $rowpegawai->bagian_shift);
					$rowx++;
                }
                $spreadsheet->getActiveSheet()->getColumnDimension('A')->setWidth(5);
                $spreadsheet->getActiveSheet()->getColumnDimension('B')->setWidth(30);
                ...
                $writer = new PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                $filename = "datapegawai_" . time() . "_bulanan" . "_download";
                ...
                $this->session->set_flashdata('exportpegawai', '<div class="alert alert-danger" role="alert">Untuk export dengan metode ini belum tersedia!</div>');
                redirect(base_url('exportpegawai'));
            }
        }
	}
}
