<?php if ($this->session->userdata('logged_in') == true) : ?>
        ...
        <div class="container-fluid">
            <div class="d-flex align-items-center justify-content-between small">
                <div class="text-muted">Copyright &copy; <?= date("Y"); ?><a href="<?= base_url(); ?>" class="ml-1"><?= $appname = (empty($dataapp['nama_app_absensi'])) ? 'Absensi Online' : $dataapp['nama_app_absensi']; ?></a>
                    <div class="d-inline">Powered By<a href="http://id.bhawata.com/" class="ml-1">BhawataNSP</a></div>
                </div>
    ...
    <?php if ($this->session->userdata('role_id') == 1) : ?>
        ...
        <script>
            $('#datashift').DataTable({
                "ajax": {
                    url: "<?= base_url('ajax/get_datatbl?type=datashift'); ?>",
                    type: 'get',
                    async: true,
                    "processing": true,
                    "serverSide": true,
            ...
            $('#addshift').submit(function(e) {
                ...
                var formdata = new FormData(form);
                $.ajax({
                    url: "<?= base_url('ajax/datashift?type=addshift'); ?>",
                    type: 'POST',
                    data: formdata,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(response) {
                        ...
                        if (response.success == true) {
                            ...
                            $('#datashift').DataTable().ajax.reload();
            ...
            $("#datashift").on('click', '.delete-shift', function(e) {
                e.preventDefault();
                var pgw_id = $(e.currentTarget).attr('data-pegawai-id');
                if (pgw_id === '') return;
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?= base_url('ajax/datashift?type=delshift'); ?>',
                            data: {
                                pgw_id: pgw_id
                            },
                            beforeSend: function() {
                            ...
                            error: function() {
                                swal.fire("Penghapusan Shift Gagal", "Ada Kesalahan Saat menghapus shift!", "error");
                            }
                        });
                    }
                })
            ...
            $("#datashift").on('click', '.view-shift', function(e) {
                e.preventDefault();
                var pgw_id = $(e.currentTarget).attr('data-pegawai-id');
                if (pgw_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ajax/datashift?type=viewshift'); ?>',
                    data: {
                        pgw_id: pgw_id
                    },
                    beforeSend: function() {
                    ...
                    success: function(data) {
                        ...
                        $('#viewdatashift').html(data);

                    },
                    error: function() {
                        swal.fire("Preview Pegawai Gagal", "Ada Kesalahan Saat menampilkan data shift!", "error");
                    }
                });
            });

            $("#datashift").on('click', '.edit-shift', function(e) {
                e.preventDefault();
                var pgw_id = $(e.currentTarget).attr('data-pegawai-id');
                if (pgw_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ajax/datapgw?type=edtshift'); ?>',
                    data: {
                        pgw_id: pgw_id
                    },
                    beforeSend: function() {
                    ...
                    success: function(data) {
                        ...
                        $('#editdatashift').html(data);

                        $('#editshift').submit(function(e) {
                            e.preventDefault();
                            var form = this;
                            $("#editpgw-btn").html("<span class='fas fa-spinner fa-pulse' aria-hidden='true' title=''></span> Menyimpan").attr("disabled", true);
                            var formdata = new FormData(form);
                            $.ajax({
                                url: "<?= base_url('ajax/editshiftbc?type=edtshiftalt'); ?>",
                                type: 'POST',
                                data: formdata,
                                processData: false,
                                contentType: false,
                                ...
                                success: function(response) {
                                    if (response.success == true) {
                                        ...
                                        swal.fire({
                                            icon: 'success',
                                            title: 'Edit Shift Berhasil',
                                            text: 'Edit shift sudah berhasil !',
                                            showConfirmButton: false,
                                            timer: 1500
                                        });
                                        $('#datashift').DataTable().ajax.reload();
                                        ...
                                        form.reset();
                                        $("#editshift-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                    } else {
                                        swal.close()
                                        $("#editshift-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                        $("#info-edit").html(response.messages);
                                ...
                                error: function() {
                                    swal.fire("Edit Shift Gagal", "Ada Kesalahan Saat pengeditan pegawai!", "error");
                                    $("#editpgw-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                }
                            });
                    ...
                    error: function() {
                        swal.fire("Edit Shift Gagal", "Ada Kesalahan Saat pengeditan shift!", "error");
                    }
                });
            });
        ...
        <!--CRUD Lokasi Tugas-->
        <script>
            $('#dataloktugas').DataTable({
                "ajax": {
                    url: "<?= base_url('ajax/get_datatbl?type=dataloktugas'); ?>",
                    type: 'get',
                    async: true,
                    "processing": true,
                    "serverSide": true,
